...
> 
> 2. 项目搁置的原因，需要归纳整理出来，并列入影响产出的预测变量，用于预估`产出能力`、`产出品质`。
...
**目标**：预测未来`市场需求`与`产出能力`之间的差距，采取改进措施 or 增加新的行动，缩小差距。
...
2. **年度规划**，对应的目标，建议以`月`为单位，设定目标，以`周`为单位，跟进进展
